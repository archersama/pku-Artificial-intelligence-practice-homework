...
w3 = tf.Variable(tf.random.truncated_normal([32, 32], stddev=0.1,seed=3))#w2为第三层参数
b3=tf.Variable(tf.random.truncated_normal([32], stddev=0.1,seed=3))
w4 = tf.Variable(tf.random.truncated_normal([32, 32], stddev=0.1,seed=3))#w2为第三层参数
b4=tf.Variable(tf.random.truncated_normal([32], stddev=0.1,seed=3))
w5= tf.Variable(tf.random.truncated_normal([32, 3], stddev=0.1,seed=4))#w4为第四层参数
b5=tf.Variable(tf.random.truncated_normal([3], stddev=0.1,seed=4))
lr = 0.05#学习率
train_loss_results = []
epoch = 1000#迭代次数
loss_all=0#总的损失
for epoch in range(epoch):  #数据集级别迭代
    for step,(x_train,y_train) in enumerate(train_db):  #batch级别迭代
        with tf.GradientTape() as tape:
            ...
            grads = tape.gradient(loss, [w1, b1, w2, b2, w3, b3,w4,b4,w5,b5])#分别计算w1,b1,w2,b2,w3,b3的梯度
