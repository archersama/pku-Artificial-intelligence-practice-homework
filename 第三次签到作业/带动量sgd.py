    ...
    for epoch in range(epoch):
        ...
        for step, (x_train, y_train) in enumerate(train_db):  # enmuerate 可以遍历数据对象
            with tf.GradientTape() as tape:  # with 记录计算过程
                ...
                m_b1 = beta*m_b1+(1-beta)*grads[1]
                m_w2 = beta*m_w2+(1-beta)*grads[2]
                m_b2 = beta*m_b2+(1-beta)*grads[3]
                n_w1 = learning_rate*m_w1
                n_w2 = learning_rate * m_w2
                n_b1 = learning_rate * m_b1
                n_b2 = learning_rate * m_b2
                w1.assign_sub(n_w1)
                b1.assign_sub(n_b1)
                w2.assign_sub(n_w2)
                b2.assign_sub(n_b2)
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.plot(train_loss_results, label="$ moment crossLoss $",color='red')
plt.legend()
# 绘制 Accuracy 曲线
plt.subplot(312)
plt.title('Acc Curve')
plt.xlabel('Epoch')
plt.ylabel('Acc')
plt.plot(train_accurate_results, label="$ moment cross Accuracy$",color="red")
plt.legend()
plt.subplot(313)
# 绘制 Learning_rate 曲线
...
plt.plot(range(global_step), lr, label="$ moment cross lr$",color="red")
plt.legend()
plt.show()
