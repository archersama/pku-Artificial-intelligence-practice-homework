    ...
    for epoch in range(epoch):
        ...
        for step, (x_train, y_train) in enumerate(train_db):  # enmuerate 可以遍历数据对象
            with tf.GradientTape() as tape:  # with 记录计算过程
                ...
                m_w2 = grads[2]
                m_b2 = grads[3]
                v_w1=beta*v_w1+(1-beta)*tf.pow(m_w1,2)
                v_b1=beta*v_b1+(1-beta)*tf.pow(m_b1,2)
                v_w2=beta*v_w2+(1-beta)*tf.pow(m_w2,2)
                v_b2= beta*v_b2 + (1 - beta) * tf.pow(m_b2, 2)
...
plt.xlabel('Epoch')
plt.ylabel('Acc')
plt.plot(train_accurate_results, label="$Accuracy$")
plt.legend()
plt.subplot(313)
# 绘制 Learning_rate 曲线
