    ...
    for epoch in range(epoch):
        ...
        for step, (x_train, y_train) in enumerate(train_db):  # enmuerate 可以遍历数据对象
            with tf.GradientTape() as tape:  # with 记录计算过程
                ...
                v_w2 += tf.pow(m_w2, 2)
                v_b2 += tf.pow(m_b2, 2)
                n_w1 = learning_rate*m_w1/(tf.sqrt(v_w1)+epsilon)
                n_w2 = learning_rate * m_w2 / (tf.sqrt(v_w2) + epsilon)
                n_b1 = learning_rate * m_b1 / (tf.sqrt(v_b1) + epsilon)
                n_b2 = learning_rate * m_b2 / (tf.sqrt(v_b2) + epsilon)
                w1.assign_sub(n_w1)
                b1.assign_sub(n_b1)
                w2.assign_sub(n_w2)
                b2.assign_sub(n_b2)
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.plot(train_loss_results, label="$ adagrad-mse-Loss $",color='blue')
plt.legend()
# 绘制 Accuracy 曲线
plt.subplot(312)
plt.title('Acc Curve')
plt.xlabel('Epoch')
plt.ylabel('Acc')
plt.plot(train_accurate_results, label="$adagrad-mse-Accuracy$",color="blue")
plt.legend()
plt.subplot(313)
# 绘制 Learning_rate 曲线
...
plt.plot(range(global_step), lr, label="$adagrad-mse-lr$",color="blue")
plt.legend()
plt.show()
